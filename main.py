...
def evaluate_model():
    ...
    with sess.as_default():
        for _ in range(config.n_eval_times):
            val_loss, val_cer, val_wer = validation_loop(
                sess,
                val_g,
                val_epoch_size,
                chars=chars,
            ...
            out_str = (
                "lm={}, beam={}, bs={:d}, test_aug:{:d}, horflip {}:"
                " CER {:.4f}, WER {:4f}\n".format(
                    config.lm_path,
                    config.beam_size,
                    config.batch_size,
...
def validation_loop(sess, g, n_batches, chars=None, val_gen=None, tb_writer=None):
    ...
    for i in range(n_batches):
        ...
        if not "infer" in config.graph_type:
            ...
            if tile_preds:
                prev_inp = np.tile(preds, [config.test_aug_times, 1])
            else:
                prev_inp = preds
            ...
            for j in range(config.maxlen):
                ...
                prev_inp = (
                    np.tile(preds, [config.test_aug_times, 1]) if tile_preds else preds
                )
...
def init_models_and_data(istrain):
    ...
    if config.lm_path:
        ...
        beam_batch_size = 1
        lm_handle = CharRnnLmWrapperSingleton(
            batch_size=beam_batch_size, sess=sess, checkpoint_path=config.lm_path
        )
