...
class TransformerInferenceGraph(TransformerTrainGraph):
  ...
  """
    ...
    def decoder_body(self, enc, dec, is_training, top_scope=None):
        ...
        # fmt: off
        symbols_to_logits_fn = self.get_symbols_to_logits_fun(enc, timing_signal, is_training, top_scope)
        # fmt: on
        ...
        if config.beam_size > 1:  # Beam Search
            ...
            if config.top_beams == 1:
                ...
                if StrictVersion(tf.__version__) >= StrictVersion("1.5"):
                    decoded_ids.decoded_ids.__dict__["_shape_val"] = tf.TensorShape(
            ...
            def inner_loop(i, next_id, decoded_ids, decoded_logits, cache):
                ...
                decoded_logits = tf.concat([decoded_logits, tf.expand_dims(logits, axis=1)], axis=1)
                # fmt: on
    ...
    def initialize_cache(self, batch_size, enc):
        ...
        for layer in cache:
            if StrictVersion(tf.__version__) >= StrictVersion("1.5"):
                cache[layer]["k"].__dict__["_shape_val"] = tf.TensorShape(
                    [None, None, key_channels]
                )
                cache[layer]["v"].__dict__["_shape_val"] = tf.TensorShape(
    ...
    def get_symbols_to_logits_fun(self, enc, timing_signal, is_training, top_scope):
        def symbols_to_logits_fn(ids, i, cache):
            ...
            # fmt: off
            pre_logits, cache = self.decoder_step(targets, cache, is_training, reuse_loop=reuse_loop)
            # fmt: on
