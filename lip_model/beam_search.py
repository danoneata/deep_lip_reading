...
def _merge_batch_into_time_dim(tensor):
    batch_size, beam_size, t, c = shape_list(tensor)
    return tf.reshape(
        tf.transpose(tensor, [1, 0, 2, 3]), [1, beam_size, batch_size * t, c]
    )


def _unmerge_batch_from_time_dim(tensor, batch_size):
    _, bm_s, bs_times_t, c = shape_list(tensor)
    t = bs_times_t // batch_size
    return tf.transpose(tf.reshape(tensor, [bm_s, batch_size, t, c]), [1, 0, 2, 3])
...
def _expand_to_beam_size(tensor, beam_size):
  ...
  """
    ...
    # TODO: This is a hack, should be updated from latest T2T
    from distutils.version import StrictVersion

    if StrictVersion(tf.__version__) >= StrictVersion("1.5"):
        shapes = shape_list(tiled_tensor)
        new_shapes = [dim if dim > 0 else None for dim in shapes]
        tiled_tensor.__dict__["_shape_val"] = tf.TensorShape(new_shapes)

    return tiled_tensor


def log_prob_from_logits(logits, axis=2):
...
def compute_batch_indices(batch_size, beam_size):
  ...
  """
    batch_pos = tf.range(batch_size * beam_size, dtype=tf.int64) // beam_size
...
def compute_topk_scores_and_seq(
  ...
  """
    ...
    topk_indexes = tf.to_int64(topk_indexes)
    ...
    if states_to_gather:
        topk_gathered_states = gather_nd_states(
            states_to_gather,
            top_coordinates,
            batch_size_states,
        ...
        #   lambda state: tf.gather_nd(state, prefix+"_topk_states"), states_to_gather)
    else:
        topk_gathered_states = states_to_gather
    return topk_seq, topk_gathered_scores, topk_flags, topk_gathered_states


def gather_nd_states(states, gather_indices, batch_size_states, name=None):
    """
  """

    # HACK: For test time augmentation, we pack the augmentation-batch into the time
    #       dim in order to select everything together for this parent
    # pack [batch, beam, time, c] -> [beam, batch*time, c]
    using_lm = "lm_state" in states
    if config.test_aug_times:
        # pack [batch, beam, time, c] -> [beam, batch*time, c]
        if using_lm:
            # The LM_states are not replicated, so we need to excluded them from the packing
            lm_state = states["lm_state"]
            del states["lm_state"]
        states = nest.map_structure(_merge_batch_into_time_dim, states)
        if using_lm:
            states["lm_state"] = lm_state

    states = nest.map_structure(
        lambda state: tf.gather_nd(state, gather_indices, name=name), states
    )

    # unpack [beam, batch*time, c] -> [batch, beam, time, c]
    if config.test_aug_times:
        if using_lm:
            lm_state = states["lm_state"]
            del states["lm_state"]
        states = nest.map_structure(
            lambda state: _unmerge_batch_from_time_dim(state, batch_size_states), states
        )
        if using_lm:
            states["lm_state"] = lm_state
...
def beam_search(
  ...
  """
    # batch_size_states = shape_list(initial_ids)[0]
    ...
    finished_seq = tf.zeros(shape_list(alive_seq), tf.int64)
    ...
    """
        ...
        finished_seq = tf.concat(
            [finished_seq, tf.zeros([batch_size, beam_size, 1], tf.int64)], axis=2
        )

        # Set the scores of the unfinished seq in curr_seq to large negative
        # values
        curr_scores += (1.0 - tf.to_float(curr_finished)) * -INF
    ...
    def grow_topk(i, alive_seq, alive_log_probs, states):
        ...
        if states:
            ...
            if not lm_symbols_to_logprobs_fn is None:
                flat_lm_logprobs, flat_states = lm_symbols_to_logprobs_fn(
                    flat_ids, i, flat_states
                )

            states = {}
            for key, val in flat_states.items():
                bs_unmerge = batch_size if key == "lm_state" else batch_size_states
                new_val = nest.map_structure(
                    lambda t: _unmerge_beam_dim(t, bs_unmerge, beam_size), val
                )
                states[key] = new_val
            # states = nest.map_structure(
            #   lambda t: _unmerge_beam_dim(t, batch_size_states, beam_size), flat_states)
        else:
        ...
        if not lm_symbols_to_logprobs_fn is None:
            lm_logprobs = tf.reshape(flat_lm_logprobs, [batch_size, beam_size, -1])
            candidate_log_probs += (
                lm_alpha * lm_logprobs
            )  # add the lm logprobs to the data ones
        ...
        # -- Here we gather the states of the parents that lead to the best hypotheses ----
        if states:
            states = gather_nd_states(states, topk_coordinates, batch_size_states)
    ...
    """
        ...
        max_length_penalty = tf.pow(((5.0 + tf.to_float(decode_length)) / 6.0), alpha)
        ...
        lowest_score_of_fininshed_in_finished += (
            1.0 - tf.to_float(tf.reduce_any(finished_in_finished, 1))
        ) * -INF
    ...
    (
        ...
        shape_invariants=[
            ...
            nest.map_structure(lambda tensor: tf.TensorShape(tensor.shape), states),
        ],
