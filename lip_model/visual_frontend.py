...
from lip_model.preproc_and_aug import (
    resize_no_crop,
    resize_vids,
    random_crop_frames,
    random_hor_flip_frames,
    normalize_mean_std,
    replicate_to_batch,
)
...
class VisualFrontend:
    def __init__(self, input):
        ...
        self.aug_out = self.preprocess_and_augment(
            model, aug_opts=aug_opts, flip_prob=flip_prob
        )
        model = self.aug_out

        if config.test_aug_times:
            self.aug_out = tf.concat([no_aug_out, self.aug_out], 0)
            model = self.aug_out

        # spatio-temporal frontend
        model = tf.contrib.keras.layers.ZeroPadding3D(padding=(2, 3, 3))(model)
        model = tf.layers.Conv3D(
            filters=64,
            kernel_size=(5, 7, 7),
            strides=[1, 2, 2],
            padding="valid",
    ...
    def preprocess_and_augment(self, input_tens, aug_opts, flip_prob=0.5):
        ...
        if not config.img_channels == 1:
            ...
            ), "Input video channels should be either 3 or 1"
            output = tf.image.rgb_to_grayscale(output)

        if config.resize_input:
            new_h = config.resize_input
            new_w = config.resize_input
            self.aug_resize = resize_no_crop(output, new_h, new_w)
            output = self.aug_resize
        ...
        if config.scale:
            self.aug_scale = resize_vids(output, scale=config.scale)
            output = self.aug_scale

        output = layers.Cropping3D(cropping=((0, 0), (crp_t, crp_b), (crp_l, crp_r)))(
            output
        )
