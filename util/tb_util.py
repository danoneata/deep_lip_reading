...
def py_encode_gif(im_thwc, tag, fps=4, timeline=False, attention=[], preds=[]):
    ...
    clip.write_gif(fname, verbose=False)
...
def add_time_line(vid, width=4):
  ...
  """
    ...
    for t in range(time_dim):
        ...
        if fill_width > 0:
            img[:width, :fill_width] = np.expand_dims(
                np.expand_dims([255, 255, 255], 0), 0
            )
    ...
    for (from_t, to_t), txt in subs:
        ...
        for t in range(from_t, to_t):
            ...
            (text_width, text_height) = cv2.getTextSize(
                txt, font, fontScale=font_scale, thickness=1
            )[0]
            posx = (w - text_width) // 2  # place subtitle in the middle
            posy = h - 10
            box_coords = (
                (posx, posy + 1),
                (posx + text_width - 2, posy - text_height - 2),
            )
...
def align_subs_from_attention_matrix(attention, preds):
    def find_best_path(grid_orig):
        """Simple dynamic progarm to find the highest score path from upper
    ...
    path_mask[list(zip(*best_path))] = 1
    subs = []
    for fr_dec, to_dec, txt in zip(frs, tos, words):
        tups = [
            t_enc for t_enc, t_dec in best_path if t_dec >= fr_dec and t_dec < to_dec
        ]
        fr_enc = min(tups)
        to_enc = max(tups)
